...
# 配置文件路径 - 支持环境变量
CONFIG_PATH = os.getenv('EODO_CONFIG_PATH', '/eodo/config.yaml')
LOG_FILE = os.getenv('EODO_LOG_FILE', '/eodo/logs.log')
...
def init_default_config():
    ...
    try:
        ...
        default_config = {
            ...
            'EdgeOneZoneId': [],
            'EdgeOneGroupName': hostname,
            'DnsPodRecord': [],
            'DingTalkWebhook': '',
            'SelectIface': '',
            'IntervalMin': 15,
            'IPv6ValidityCheck': True,
            'IPv6SelectMode': 'auto',
...
class QcloudClient:
    ...
    def describe_origin_group(self, zone_id, group_name=None):
        if group_name is None:
            group_name = hostname
        body = {"ZoneId": zone_id, "Filters": [{"Name": "origin-group-name", "Values": [group_name]}]}
        response = requests.post(
            f'https://{self.host}', headers=self.signature('DescribeOriginGroup', body), json=body
        ).json()
        return response.get('Response', {}).get('OriginGroups', {})

    def create_origin_group(self, zone_id, iplist, group_name=None):
        if group_name is None:
            group_name = hostname
        body = {"ZoneId": zone_id, "Name": group_name, "Type": "HTTP",
...
# =================== 工具类 ===================
class IPv6Tool:
    def __init__(self, select_iface="", task_id="", ipv6_validity_check=True):
        self.task_id = task_id
        self.ipv6_validity_check = ipv6_validity_check
        ...
        for iface, addr_list in addrs.items():
            ...
            for addr in addr_list:
                ...
                if addr.family == socket.AF_INET6 and self.is_public_ipv6(ip):
                    # 如果启用了IPv6有效性检测，则进行连通性检查
                    if self.ipv6_validity_check:
                        if self.public_ipv6_check(ip):
                            ipv6_list.append(ip)
                        else:
                            continue
                    else:
                        # 如果未启用检测，直接添加所有公网IPv6地址
                        ipv6_list.append(ip)
    ...
    def get_all_public_ipv6_list(self, select_iface=""):
        """获取所有公网IPv6地址，不进行连通性检查"""
        ipv6_list = []
        addrs = psutil.net_if_addrs()
        for iface, addr_list in addrs.items():
            if select_iface and iface != select_iface:
...
def update_task(task_id=""):
    config = read_config()
    ipv6_validity_check = config.get("IPv6ValidityCheck", True)
    ipv6_select_mode = config.get("IPv6SelectMode", "auto")
    selected_ipv6_addresses = config.get("SelectedIPv6Addresses", [])
    ...
    if ipv6_select_mode == "manual" and selected_ipv6_addresses:
        ...
        public_ipv6 = iptool.public_ipv6
        
        # 如果连通性检查失败，尝试获取所有公网IPv6地址
        if not public_ipv6:
            all_ipv6_addresses = iptool.get_all_public_ipv6_list(config.get("SelectIface", ""))
            if all_ipv6_addresses:
                public_ipv6 = set(all_ipv6_addresses)
                logger.info(f"[{task_id}] 连通性检查失败，使用所有检测到的 IPV6 地址：{",".join(public_ipv6)}")
            else:
    ...
    if eo_zones:
        ...
        for zone in eo_zones:
            ...
            if len(origin_groups) >= 1:
                ...
                if public_ipv6 == records:
                    logger.info(f"[{task_id}] 公网 IPV6 地址未发生变更，站点 {zone} 的源站组 {edge_one_group_name} 无需更新。")
                else:
                    logger.info(f"[{task_id}] 公网 IPV6 地址发生变更，新的地址： {public_ipv6}")
                    error_msg, error_code = eo_client.modify_origin_group(zone, group_id, public_ipv6)
                    error_msg = F"成功更新站点 {zone} 的源站组 {edge_one_group_name} 。" if not error_code and not error_msg else error_msg
                    logger.info(f"[{task_id}] {error_msg} {error_code}")
                    dingtalk.notice_eo_result(edge_one_group_name, zone, list(public_ipv6), error_msg)
            else:
                logger.info(f"[{task_id}] 站点 {zone} 的源站组 {edge_one_group_name} 尚未未创建。")
                error_msg, error_code = eo_client.create_origin_group(zone, public_ipv6, edge_one_group_name)
                error_msg = F"成功创建站点 {zone} 的源站组 {edge_one_group_name} 。" if not error_code and not error_msg else error_msg
                logger.info(f"[{task_id}] {error_msg} {error_code}")
                dingtalk.notice_eo_result(edge_one_group_name, zone, list(public_ipv6), error_msg)
    ...
    if domains:
        ...
        for domain in domains:
            ...
            for record in records:
                if record["Value"] not in list(public_ipv6):
            ...
            if record_counts >= 1:
                ...
                error_msg, error_code = dnspod.create_dns_record(top_domain, sub_domain, record_type, public_ipv6)
                error_msg = f"成功更新解解析记录 {fqdn} " if not error_code and not error_msg else error_msg
                logger.info(f"[{task_id}] {error_msg} {error_code}")
                dingtalk.notice_dns_result(fqdn, list(public_ipv6), error_msg)
...
class TaskScheduler:
    ...
    def start_scheduler(self):
        if self.scheduler_thread is None or not self.scheduler_thread.is_alive():
            ...
            logger.info("IPv6同步服务已启动")

    def stop_scheduler(self):
        if self.scheduler_thread and self.scheduler_thread.is_alive():
            self.scheduler_stop_flag.set()
            self.is_running = False
            logger.info("IPv6同步服务已停止")
    ...
    def get_status(self):
        return {
            "running": self.is_running and (self.scheduler_thread and self.scheduler_thread.is_alive()),
...
def api_detect_ipv6():
    ...
    try:
        ...
        ipv6_tool = IPv6Tool(select_iface, "", ipv6_validity_check)
        
        # 首先尝试获取通过连通性检测的IPv6地址
        if ipv6_tool.public_ipv6:
            return {"addresses": list(ipv6_tool.public_ipv6)}
        else:
            # 如果连通性检测失败，返回所有检测到的公网IPv6地址
            all_ipv6_addresses = ipv6_tool.get_all_public_ipv6_list(select_iface)
            if all_ipv6_addresses:
                logger.info(f"连通性检测失败，但检测到 {len(all_ipv6_addresses)} 个公网IPv6地址")
                return {"addresses": all_ipv6_addresses, "connectivity_failed": True}
            else:
                return {"addresses": []}
    except Exception as e:
        logger.error(f"IPv6地址检测失败: {str(e)}")
...
async def control_service(request: Request):
    ...
    if action == "start":
        ...
        return {"msg": "IPv6同步服务已启动", "status": "running"}
    elif action == "stop":
        scheduler.stop_scheduler()
        return {"msg": "IPv6同步服务已停止", "status": "stopped"}
    else:
        return {"msg": "无效的操作", "error": True}, 400
